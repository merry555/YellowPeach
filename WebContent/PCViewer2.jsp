			...
			for (i = 0; i < objlen; i++) {
				for (j = 0; j < numlen; j++) {
					if((control[i][j]) && (viewList[i][j].chart == "camera")) {
						...
						$cframe = $("<div class='camera' > <iframe src='http://192.168.0.27:8000/image.jpg' width='340px' height='170px' name ='123'></iframe></div>");
						$("#draw"+j).replaceWith($cframe);
					}
					else {
						console.log("컴트롤" + control[i][j]);
						console.log("zkapf" + viewList[i][j].chart);
						console.log("nothing");
					}
				}	
			}
				...
				for(j=0; j<numlen; j++) {
					...
					if(viewList[i][j].chart == "video") {
						...
						$frame0 = $("<div id='size0'>"+ viewList[i][j].chartName
								...
								+ strValue1 + "&value2=" + strValue2 + "&cont1=" + strControl1
								+ "&cont2=" + strControl2 +"'><i class='fa fa-fw fa-plus''></i></a></div>"
								+"<div id='size'> <img src ='./img/stream.png' width='340px' height='200px'/></div>");
